...
public interface TransactionDao extends JpaRepository<ModelTransaction, Integer>{
	...
	@Query(value="select trs_id,status,payment_method,merchant_name,date_time,expenditure,pg_ref_id from transactions where (Date(date_time) between ?1 and ?2)", nativeQuery=true)
	...
	@Query(value="select trs_id,status,payment_method,merchant_name,date_time,expenditure,pg_ref_id from transactions where (Date(date_time) between ?2 and ?1)", nativeQuery=true)
	...
	@Query(value="select trs_id,status,payment_method,merchant_name,date_time,expenditure,pg_ref_id from transactions", nativeQuery=true)
